...
void lock_init_p(int lock)
{
	invoke_syscall(SYSCALL_LOCK_INIT, (int) lock,IGNORE,IGNORE);
}
int  lock_acquire_p(int lock)
{
	return (int) invoke_syscall(SYSCALL_LOCK_ACQUIRE, (int) lock, IGNORE, IGNORE);
}

void lock_release_p(int lock)
{
	invoke_syscall(SYSCALL_LOCK_RELEASE, (int) lock, IGNORE, IGNORE);
}

int get_test_lock_p(void)
{
	invoke_syscall(SYSCALL_GET_TEST_LOCK, IGNORE, IGNORE, IGNORE);
}
