...
static void initialize_pcb(pcb_t *p, pid_t pid, struct task_info *ti)
{
	...
	p->nested_count = 1-ti->task_type;
	p->entry_point = ti->entry_point;
	p->pid = pid;
	p->task_type = ti->task_type;
	p->status = FIRST_TIME;
	p->entry_count = 0;
	p->blocking_lock = (void *)NULL;
	p->kernel_tf.regs[0] = 0;
	...
	p->user_tf.regs[31] = (uint32_t) &first_entry;
	p->user_tf.cp0_status = 0x10008000;
	p->user_tf.cp0_cause = 0;
	p->user_tf.cp0_epc = ti->entry_point;
	p->priority = pid + 1;
	p->now_left = p->priority;
