...
void scheduler(){
     ...
     //printf(20,1,"get here 1");
     if (current_running->task_type == PROCESS){
		//printf(23,1,"cur_pagetable is 0x%x",current_running->page_table);
		set_pt();
	 }
	 //printf(21,1,"get here 2");
     ASSERT(NULL != current_running);
	 //printf(22,1,"get here 3");
     ++current_running->entry_count;
	 printf(25+current_running->pid,1,"pid %d,epc: 0x%x , kpc: 0x%x,upc: 0x%x ksp: 0x%x,usp : 0x%x",current_running->pid,current_running->kernel_tf.cp0_epc,current_running->kernel_tf.regs[31],current_running->user_tf.regs[31],current_running->kernel_tf.regs[29],current_running->user_tf.regs[29]);
	 //print_all_tlb();
}
