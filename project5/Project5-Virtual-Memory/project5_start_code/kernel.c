...
static void      initialize_proc(pid_t pid , struct task_info *ti);
...
void __attribute__( ( section( ".entry_function" ) ) ) _start( void ) {
	...
	//current_running->kernel_stack = (uint32_t) stack_new();
...
static void initialize_pcb( pcb_t *p, pid_t pid, struct task_info *ti ) {
    ...
    case PROCESS:
        ...
        p->kernel_tf.regs[ 29 ] = (uint32_t) stack_new();
		if (on_demand)
        	p->user_tf.regs[ 29 ]   = 0x2fff00;
        	//p->user_tf.regs[ 29 ]   = 0x300000;
		else
			p->user_tf.regs[ 29 ]   = (uint32_t) stack_new();
        p->nested_count         = 0;
        // TODO: p5 here!
        if (pid == 1)
			p->disk_place = 0xa0b00000;
		else
			p->disk_place = 0xa0d00000;
        p->size                 = ti->size;
        p->loc                  = ti->loc;
        p->page_table           = setup_page_table(pid);
		p->stack                = p->user_tf.regs[ 29 ];
        break;
    default:
        ASSERT( FALSE );
	//p->kernel_stack = (uint32_t) stack_new();
    }
...
static void first_entry() {
	...
	//printf(1,1,"get first_entry,pid = %d , stack = 0x%x,entry_point = 0x%x,disable count = %d",current_running->pid,stack,entry_point,disable_count);
...
static void init_syscalls() {
    ...
    syscall[ SYSCALL_GETPRIORITY ]  = ( int ( * )() ) &do_getpriority;
...
void print_status( void ) {
    ...
    printf( base - 2, 1, "Pid\tType\tPrio\tStatus\tEntries\tTLB miss\tPage fault" );
    for ( i = 0; i < NUM_PCBS && ( base + i ) < 25; i++ ) {
        printf( base + i, 1, "%d\t%s\t%d\t%s\t%d\t%d\t%d", pcb[ i ].pid,
                pcb[ i ].task_type == KERNEL_THREAD ? "Thread" : "Process",
                pcb[ i ].priority, status[ pcb[ i ].status ],
                pcb[ i ].entry_count,pcb[i].tlb_miss,pcb[i].page_fault );
    }
}
