...
#define TIMER_INTERVAL 2000000000
...
extern void handle_tlb_miss_start();
extern void handle_tlb_miss_end();

extern void handle_int();
extern void handle_tlb_miss();
...
void init_interrupts(void) {
  int i = 0;
  for (i = 0; i < 32; ++i) {
    interrupt_handlers[i] = (uint32_t)simple_handler;
  }
  interrupt_handlers[INT] = (uint32_t)handle_int;
  interrupt_handlers[SYS] = (uint32_t)handle_syscall;
  interrupt_handlers[TLBL] = (uint32_t)handle_tlb;
  interrupt_handlers[TLBS] = (uint32_t)handle_tlb;
  ...
  printk("tlb_miss_start : %x\n", handle_tlb_miss_start);
  printk("tlb_miss_end : %x\n", handle_tlb_miss_end);
  // fill nop
  bzero((char *)EBASE, EBASE_OFFSET);
  //bcopy((char *)handle_tlb_miss_start, (char *)EBASE+0x200,
  //      handle_tlb_miss_end - handle_tlb_miss_start);
  // copy the exception handler to EBase
  bcopy((char *)exception_handler_start, (char *)EBASE + EBASE_OFFSET,
        exception_handler_end - exception_handler_start);
  // When BEV=0, EBASE change to 0x80000000
  // offset change to 0x180
  bzero((char *)0x80000000, 0x180);
  //bcopy((char *)handle_tlb_miss_start,(char *) 0x80000000,
  //      handle_tlb_miss_end - handle_tlb_miss_start);
  bcopy((char *)exception_handler_start,(char *) 0x80000180,
        exception_handler_end - exception_handler_start);
