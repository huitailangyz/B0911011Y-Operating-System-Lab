...
uint32_t get_dir_idx(uint32_t vaddr){
	return (vaddr & PAGE_DIRECTORY_MASK) >> 22;
...
void set_ptab_entry_flags(uint32_t * pdir, uint32_t vaddr, uint32_t flag){
	uint32_t dir_idx = get_dir_idx((uint32_t) vaddr);
...
void insert_ptab_dir(uint32_t * dir, uint32_t *tab, uint32_t vaddr, uint32_t flag){
	uint32_t access = flag & MODE_MASK;
...
uint32_t *setup_page_table( int pid ) {
	...
	uint32_t pstart=p->entry_point;
	uint32_t process_end=pstart+p->size;
